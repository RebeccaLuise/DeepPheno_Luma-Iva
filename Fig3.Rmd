...
p3 <- metadata_complete%>%
  filter(!is.na(GOT), visit!=6) %>% 
  ggplot(aes(x=visit_wo8, y=GOT)) +
  theme_minimal()+
  geom_point(aes(color = as_factor(id)), size=2.5) +
  geom_line(aes(x = visit_wo8, group = as_factor(id), color = as_factor(id),alpha=0.7), size=3) +
  geom_boxplot(aes(alpha=0.7), outlier.colour = NA) +
...
ggsave(plot=grid_arranged, "~/Documents/Forschung/Orkambi_Kids/Paper/DeepPheno_Luma-Iva/gastro_fig.pdf", width = 12, height = 12)
```
...
lm_stats_function_wo8 <- function(data, variable_of_interest, label) {
  lm <- summary(lmerTest::lmer({{ variable_of_interest }} ~ visit_wo8 + sex + age + (1|id), data = data))
#lm <- summary(lmerTest::lmer(GOT ~ visit_wo8 + sex + age + (1|id), data = metadata_complete))
  ...
  lm_stats <- bind_cols(coefs, fdr) %>%
    ...
    mutate(Months_after_ETI_start = case_when(
      Months_after_ETI_start == "(Intercept)" ~ "Baseline (Intercept)",
      Months_after_ETI_start == "visit_wo82" ~ "3",
      Months_after_ETI_start == "visit_wo83" ~ "6",
      Months_after_ETI_start == "visit_wo84" ~ "9",
      Months_after_ETI_start == "visit_wo85" ~ "12",
      Months_after_ETI_start == "visit_wo87" ~ "18",
      Months_after_ETI_start == "visit_wo89" ~ "24",
      Months_after_ETI_start == "sex" ~ "sex",
      Months_after_ETI_start == "age" ~ "age_y"
    )) %>%
    mutate(fdr_star = case_when(
...
# Print lm stats into data frame and add N of observations
  nobs_per_visit <- data %>%
    ...
    summarise(N_obs = n()) %>% 
    mutate(Months_after_ETI_start = case_when(
      visit_wo8 == "1" ~ "Baseline (Intercept)",
      visit_wo8 == "2" ~ "3",
      visit_wo8 == "3" ~ "6",
      visit_wo8 == "4" ~ "9",
      visit_wo8 == "5" ~ "12",
...
asat_stats <- lm_stats_function_wo8(metadata_complete, metadata_complete$GOT, "ASAT [U/I]")
asat_stats

alat_stats <- lm_stats_function_wo8(metadata_complete, metadata_complete$GPT, "ALAT [U/I]")
alat_stats

# combine stats table
systemic_stats <- rbind(bmi_stats,cal_stats,asat_stats,alat_stats)

write_csv(systemic_stats, "/Users/rebecca/Documents/Forschung/Orkambi_Kids/Paper/Suppl_tables/st6_gastro_responses_stats.csv")
```
...
# filter metadata for data that matches the stool samples analyzed
metadata<- metadata_complete %>% 
  mutate(st_id = paste("19", substr(grouping, 1, 4), "St", sep = "")) %>% 
  mutate(st_id= case_when(st_id == "19SAV1St"~ "19SAV1StA", T~st_id)) %>% 
  filter(st_id %in% colnames(BC_df))

vegan::adonis2(BC_dist ~ visit+id+pp_fev1, by="margin", permutations = 999, strata = metadata$id, data = metadata) # stratified for id
vegan::adonis2(BC_dist ~ visit+id, by="margin", permutations = 999, data = metadata) # non-stratified
vegan::adonis2(BC_dist ~ visit+id+pp_fev1, by="margin", permutations = 999, data = metadata)# non-stratified for id
vegan::adonis2(BC_dist ~ visit+id+pp_fev1, by="margin", permutations = 999, strata = metadata$id, data = metadata) # stratified for id

# include antibiotic treatment in PERMANOVA


vegan::adonis2(BC_dist ~ visit+id+pp_fev1+ab365vorher+ab15vorher, by="margin", permutations = 999, strata = metadata$id, data = metadata, na.action = na.exclude) # stratified for id

vegan::adonis2(BC_dist ~ visit+id+pp_fev1+ab365vorher+ab15vorher, by="margin", permutations = 999,  data = metadata, na.action = na.exclude) # non-stratified for id

## impact of antibiotic treatment days prior visit?
metadata %>% 
  ggplot(aes(y=ab365vorher, x=visit))+
  geom_boxplot()+
  geom_point(aes(color=id))+
  scale_y_log10()

metadata %>% 
  ggplot(aes(y=ab15vorher, x=visit))+
  geom_boxplot()+
  geom_point(aes(color=id))+
  scale_y_log10()

antb_stats <- lm_stats_function(metadata_complete, metadata_complete$ab365vorher, "N atb tt days")
antb_stats

# ppfev1 and id are highly linked, thus when stratifying for id, ppfev1 seems to have a significant impact, but when not stratifiyng for id, id explains all of the variance and ppfev1 is not significant.
