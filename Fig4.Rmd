...
sample_data(ps_orkambi_work_rel)$visit_id <- paste(sample_data(ps_orkambi_work_rel)$visit, sample_data(ps_orkambi_work_rel)$id, sep = "_")
...
p1 <- metadata_complete %>%
  ...
  scale_x_discrete(labels = c(0, 3, 6, 9, 12, 15, 18, 21, 24))+ # Custom x-axis labels
...
p8 <- metadata_complete%>%
  filter(visit %in% c("1","2","3","4","5")) %>%
  ggplot(aes(x=visit, y= ne)) +
  theme_minimal()+
  geom_point(aes(color = as_factor(id)), size=2.5) +
  geom_line(aes(x = visit, group = as_factor(id), color = as_factor(id),alpha=0.7), size=3) +
  geom_boxplot(aes(alpha=0.7), outlier.colour = NA) +
  ylab("Neutrophile elastase [g/l]")+
  theme(text = element_text(size=16), legend.position = "none") +
  scale_color_manual(name = "id", values = my_colors_id)+
  scale_y_log10()+
  scale_x_discrete(labels = c(0, 3, 6, 9, 12, 15, 18, 21, 24))+ # Custom x-axis labels
...
p7 <- metadata_complete%>%
  filter(visit %in% c("1","2","3","4","5")) %>%
  ggplot(aes(x=visit, y= tn_falpha)) +
  theme_minimal()+
  geom_point(aes(color = as_factor(id)), size=2.5) +
  geom_line(aes(x = visit, group = as_factor(id), color = as_factor(id),alpha=0.7), size=3) +
  geom_boxplot(aes(alpha=0.7), outlier.colour = NA) +
...
p6 <- metadata_complete%>%
  filter(visit %in% c("1","2","3","4","5")) %>%
  ggplot(aes(x=visit, y= il6)) +
  theme_minimal()+
  geom_point(aes(color = as_factor(id)), size=2.5) +
  geom_line(aes(x = visit, group = as_factor(id), color = as_factor(id),alpha=0.7), size=3) +
  geom_boxplot(aes(alpha=0.7), outlier.colour = NA) +
  ylab("IL-6 [pg/ml]")+
  guides(color = guide_legend(title = "id", nrow=1, byrow=TRUE, title.position = "left"), alpha="none")+
...
inf_all<- ggarrange(p7,p8,p6, ncol = 3, legend.grob = get_legend(p6), labels = c("A","B","C"))
 inf_all
ggsave("~/Documents/Forschung/Orkambi_Kids/Paper/DeepPheno_Luma-Iva/suppFig_inflammation_sputum.png",inf_all,dpi = 300, width = 10, height = 4)
...
ps_paired <- subset_samples(ps_resp_rel, visit_id%in%sample_data(ps_orkambi_sputum_rel)$visit_id)
ps_paired <- subset_samples(ps_paired, visit_id%in%sample_data(ps_orkambi_throat_rel)$visit_id)
set.seed(100)
...
# margin
v1 <- vegan::adonis2(BC_dist ~ material + visit + id + dominantGenus+ pp_fev1+ ab365vorher+ab15vorher,
              permutations = 999, na.action=na.exclude, data = metadata, by="margin", strata = metadata$id)

v2 <- vegan::adonis2(BC_dist ~ material + visit + id + dominantGenus+pp_fev1+ab365vorher+ab15vorher,
              permutations = 999, na.action=na.exclude, data = metadata, by="margin") # non-stratified for id

# plot PERMANOVA as pie chart of variance explained

v1_df <- clean_names(as_tibble(v1)) # I take the non-stratified for id, because it's important to highlight the impact of id 
v1_df$variable<-rownames(v1_df)
variable_name <- c("sample type" , "sample time point" , "id" , "dominant genus", "pp_fev1", "ab365" , "ab15", "residual", "Total")
...
v1_df <- v1_df%>%
  mutate(variable=variable_name) %>% 
  mutate(p = case_when(variable=="residual" ~ 0.051, variable=="Total" ~ 0.051, TRUE~pr_f))

# plot R2 only of variables that have a p < 0.05

label_name <- c("sample type", "sample time point", "id", "dominant genus", "pp_fev1", "ab365" , "ab15", "residual")
fill_values <- c("#FF5733", "#2E8B57", "#FFD700", "#919C4C", "#4682B4", "#D2691E", "#BAB0AC", "#41EAD4")
...
p4 <- v1_df %>%
  ...
  mutate(text_y = cumsum(percent) - percent / 2) %>%
  filter(p <= 0.05) %>%
  ggplot(aes(x = fct_reorder(variable, percent), y = percent, fill = fct_reorder(variable, percent))) +
  geom_col(width = 0.7, color = "black", alpha = 0.7) +
  scale_fill_manual(values = fill_values, breaks = label_name, labels = label_name) +
  theme_minimal() +
...
# margin
v1 <- vegan::adonis2(BC_dist ~ visit + id + dominantGenus+ pp_fev1+ ab365vorher +  ab15vorher,
              permutations = 999, na.action=na.exclude, data = metadata, strata = metadata$id, by="margin")

# unstratified
v2 <- vegan::adonis2(BC_dist ~ visit + id + dominantGenus+ pp_fev1+ ab365vorher + ab15vorher,
...
v1_df$variable<-rownames(v1_df)
variable_name <- c( "sample time point" , "id" , "dominant genus", "pp_fev1", "ab365" , "ab15", "residual", "Total")
...
BC_dist<- phyloseq::distance(ps_throat_rel,
                             method="bray", weighted=F)
# extract metadata
metadata_thr <- as(sample_data(ps_throat_rel),"data.frame") # somehow this metadata misses one lung function entry, I use therefore the metadata_complete and paste it with the throat metadata
metadata_thr <-metadata_thr %>% 
  select(probe_id, dominantGenus)
BC_df <- as.matrix(BC_dist)
BC_df <- as.data.frame(BC_df)
rownames(BC_df)
# filter metadata for data that matches the throat samples analyzed
metadata <- metadata_complete %>% 
  mutate(st_id = paste("19", substr(grouping, 1, 4), "Thr", sep = "")) %>% 
  #mutate(st_id= case_when(st_id == "19SAV1St"~ "19SAV1StA", T~st_id)) %>% 
  filter(st_id %in% colnames(BC_df)) %>% 
  left_join(metadata_thr, by="probe_id")
...
# unstratified
v2 <- vegan::adonis2(BC_dist ~ visit + id + dominantGenus+ ab365vorher + ab15vorher,
              permutations = 999, na.action=na.exclude, data = metadata, by="margin")
...
grid_arranged <- grid.arrange(
  ...
  ncol = 8, nrow = 2,
  heights = c(1, 1.5),
  widths= c(1,1,1,1, 1.2,1.2, 1.2,1.2),
...
ggsave(plot = grid_arranged,"~/Documents/Forschung/Orkambi_Kids/Paper/DeepPheno_Luma-Iva/resp_fig.pdf", width = 15, height = 12)
```
...
il1b_stats <- lm_stats_function(metadata_complete, metadata_complete$il1beta, "IL-1ß in sputum")
il1b_stats
