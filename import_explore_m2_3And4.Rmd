---
title: "Serum metabolomic analysis of the OrkambiKIDS study - reduced feature space: only metabolites which have ms2: 3 and 4 annottaion (better quality)"
...
BC_dist <- phyloseq::distance(ps_HILIC_neg, method="bray", weighted=T, na.rm=T)
...
# calculate BC_distance
BC_dist <- phyloseq::distance(ps_HILIC_pos, method="bray", weighted=T, na.rm=T)
...
# calculate BC_distance
BC_dist <- phyloseq::distance(ps_RP_neg, method="bray", weighted=T, na.rm=T)
...
# calculate BC_distance
BC_dist <- phyloseq::distance(ps_RP_pos, method="bray", weighted=T, na.rm=T)
...
df_comp_relab %>%
  ...
  facet_wrap("OTU", scales="free", ncol = 5)+
  theme_bw()+
  ggtitle("100 top metabolites of PCA")+
  theme(axis.text.x = element_blank()) +
  ylab("intensity")+
  xlab("")+
  theme(legend.key.height= unit(5, 'mm'),
